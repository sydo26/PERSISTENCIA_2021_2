...
public class RegisterEmployeeSection implements Section<EmployeeDaoJPA> {
    @Override
    public boolean execute(Console console, String response, EmployeeDaoJPA repository) {
        ...
        try {
            Employee employee = repository.save(Employee.create(name, email, phone, cpf.replaceAll("[\\-\\.]", "")));
            if (employee != null) {
